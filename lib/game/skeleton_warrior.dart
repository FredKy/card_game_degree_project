...
class SkeletonWarrior extends PositionComponent
    with CollisionCallbacks, HasGameReference<CardGame>, Draggable {
  static final _paint = Paint()..color = Colors.white;
  SkeletonWarrior() : super(size: Vector2(256, 320), anchor: Anchor.center);
  ...
  void render(Canvas canvas) {
    ...
    idleAnimation.getSprite().render(canvas,
        position: Vector2(size.x / 2, size.y / 2),
        anchor: Anchor.center,
        size: skeletonSprite.srcSize.scaled(6));
  }
  ...
  static late final Sprite skeletonSprite =
      getEnemySprite(256 + 32, 48, 128 - 2 * 32, 128, "skeleton_warrior_idle");
  static late final idleAnimation = SpriteAnimation.spriteList([
    for (var i = 0; i < 7; i++)
      getEnemySprite(
          i * 128 + 32, 48, 128 - 2 * 32, 128, "skeleton_warrior_idle")
  ], stepTime: 0.125);
